...
public class ProductForm extends Div {
    ...
    public ProductForm(InventoryViewLogic sampleCrudLogic) {
        ...
        // Android omits java.beans.Introspector. Explicit bindings preserve the
        // model's constraints without reflective bean-property discovery.
        binder = new Binder<>();
        binder.forField(productName)
                .withValidator(value -> value != null && value.length() >= 2,
                        "Product name must have at least two characters")
                .bind(Product::getProductName, Product::setProductName);
        binder.forField(price).withConverter(new PriceConverter())
                .withValidator(value -> value == null || value.signum() >= 0,
                        "must be greater than or equal to 0")
                .bind(Product::getPrice, Product::setPrice);
        binder.forField(stockCount).withConverter(new StockCountConverter())
                .withValidator(value -> value >= 0, "Can't have negative amount in stock")
                .bind(Product::getStockCount, Product::setStockCount);
        binder.forField(availability).asRequired("must not be null")

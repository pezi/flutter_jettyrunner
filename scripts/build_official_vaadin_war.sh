...
mvn -B -f "$project_dir/vaadin-official-demo/pom.xml" clean package
python3 "$project_dir/scripts/package_android_war.py" \
  "$project_dir/vaadin-official-demo/target/vaadin-official-demo.war" \
  "$project_dir/war_repository/vaadin-official-demo.war" \
  --d8 "$d8" --android-jar "$android_jar" \
  --classpath "$project_dir/vaadin-official-demo/target/d8-classpath" \
  --asm-jar "$project_dir/vaadin-official-demo/target/android-tools/asm-9.9.jar"
